...
A driven adapter (Hexagonal / Ports & Adapters): implements
domain.ports.PoseMemoryPort. domain/motion_validator.py depends only on
that Protocol, not on "this is a JSON file" — a different storage
backend would be a different driven adapter, no domain change needed.
...
from ...domain.ports import PoseFact
