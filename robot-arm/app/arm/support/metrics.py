...
Purely a human-debugging aid (GET /metrics, see routes/metrics_routes.py)
— not part of the agent's tool set (SPEC.md §5.3), the same "not read by
any LLM" carve-out already established for run_history.jsonl on the
agent side. Resets to zero on container restart; unlike rejected_history
and pose_memory, there's no reason for this to survive one — it's a
running tally, not a record.
"""
