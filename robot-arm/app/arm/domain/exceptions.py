...
`error_code` the HTTP driving adapter maps onto a response body, so the
caller gets a machine-readable reason instead of having to parse a
string. Errors that have actionable structured data beyond the message
(e.g. a suggested alternative target) expose it via `details`, which the
HTTP layer merges into the response body — new error types can add
fields there without touching the HTTP layer.
"""
