...
Depends only on domain.ports.ArmPhysicsPort/PoseMemoryPort — never
imports a concrete adapter (Hexagonal / Ports & Adapters); the driven
adapters (PyBulletAdapter, PoseMemory) are wired in at composition time.

Not thread-safe on its own — touches the shared physics port via
`dry_run`. Callers must hold the same lock the port's concrete adapter
requires.
"""
...
from .ports import ArmPhysicsPort, PoseFact, PoseMemoryPort
from .util import clamp, distance


class MotionValidator:
    def __init__(self, adapter: ArmPhysicsPort, pose_memory: PoseMemoryPort) -> None:
        self._adapter = adapter
        self._pose_memory = pose_memory
    ...
    def recall_pose(self, position: List[float]) -> Optional[PoseFact]:
